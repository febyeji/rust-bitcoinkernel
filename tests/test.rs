...
mod tests {
    ...
    use bitcoinkernel::{
        ...
        VERIFY_NONE, VERIFY_TAPROOT, VERIFY_WITNESS,
    };
    ...
    /// Helper: verify the P2PKH transaction against a custom scriptPubkey hex.
    /// Reuses the same valid P2PKH spending transaction (with sig + pubkey in scriptSig).
    fn verify_with_script(script_hex: &str, flags: btck_ScriptVerificationFlags) -> Result<(), KernelError> {
        ...
        verify_test(script_hex, p2pkh_tx, 0, 0, vec![], flags)
    }
    ...
    fn test_script_error_codes() {
        ...
        assert!(matches!(
            verify_with_script("6a", VERIFY_NONE),
            Err(KernelError::ScriptVerify(ScriptVerifyError::Script(ScriptError::OpReturn)))
        ));

        // OP_0 (0x00) pushes empty bytes → top of stack is false → EvalFalse
        assert!(matches!(
            verify_with_script("00", VERIFY_NONE),
            Err(KernelError::ScriptVerify(ScriptVerifyError::Script(ScriptError::EvalFalse)))
        ));

        // OP_EQUALVERIFY (0x88) pops sig and pubkey, they differ → EqualVerify
        assert!(matches!(
            verify_with_script("88", VERIFY_NONE),
            Err(KernelError::ScriptVerify(ScriptVerifyError::Script(ScriptError::EqualVerify)))
        ));

        // OP_IF (0x63) without OP_ENDIF → UnbalancedConditional
        assert!(matches!(
            verify_with_script("63", VERIFY_NONE),
            Err(KernelError::ScriptVerify(ScriptVerifyError::Script(ScriptError::UnbalancedConditional)))
        ));

        // OP_DROP OP_DROP OP_DROP (0x75 0x75 0x75): scriptSig pushes 2 items,
        // third OP_DROP has nothing to pop → InvalidStackOperation
        assert!(matches!(
            verify_with_script("757575", VERIFY_NONE),
            Err(KernelError::ScriptVerify(ScriptVerifyError::Script(ScriptError::InvalidStackOperation)))
        ));

        // OP_2MUL (0x8d) is a disabled opcode → DisabledOpcode
        assert!(matches!(
            verify_with_script("8d", VERIFY_NONE),
            Err(KernelError::ScriptVerify(ScriptVerifyError::Script(ScriptError::DisabledOpcode)))
        ));
